...
class XQueueViewSet(viewsets.ViewSet):
    ...
    def login(self, request):
        ...
        if request.method == "GET":
            ...
            get_token(request)
            return Response(self.compose_reply(True, ""), status=status.HTTP_200_OK)
        ...
        return Response(
            self.compose_reply(True, "Logged in"), status=status.HTTP_200_OK
        )
    ...
    def logout(self, request):
        ...
        """
        logout(request)
        return Response(self.compose_reply(True, "Goodbye"), status=status.HTTP_200_OK)
    ...
    def get_submission(self, request):
        ...
        external_grader = (
            ExternalGraderDetail.objects.select_for_update(skip_locked=True)
            .filter(
                ...
                | Q(
                    ...
                    status="pulled",
                    status_time__lt=timeout_threshold,
                )
            )
        ...
        if external_grader:
            ...
            if external_grader.status != "pulled":
                try:
                    ...
                    return Response(
                        self.compose_reply(
                            False, "Unable to transition submission for grading"
                        ),
            ...
            try:
                submission_data = {
                    "grader_payload": json.dumps(
                        {"grader": external_grader.grader_file_name}
                    ),
                    "student_info": json.dumps(
                        {
                            "anonymous_student_id": str(
                                external_grader.submission.student_item.student_id
                            ),
                            "submission_time": str(
                                int(external_grader.created_at.timestamp())
                            ),
                            "random_seed": 1,
                        }
                    ),
    ...
    def put_result(self, request):
        ...
        if not reply_valid:
            log.error("Invalid reply from pull-grader: request.data: %s", request.data)
            return Response(
        ...
        try:
            ...
            external_grader = (
                ExternalGraderDetail.objects.select_for_update(nowait=True)
    ...
    def validate_grader_reply(self, external_reply):
        ...
        try:
            ...
            log.error(
                "Failed to parse grader_reply as JSON or extract score: %s. Raw grader_reply: %s",
                str(e),
                grader_reply,
            )
            return fail
    ...
    def compose_reply(self, success, content):
        ...
        """
        return {"return_code": 0 if success else 1, "content": content}
